...
n_Pt=$(awk '{if($1=="Pd"){n+=1} else {n+=0}} END {print n}' init_xyz/$i.xyz)

T=$(echo $n_Au $n_Pd | awk -v f=$f '{print f*($1/($1+$2)*750.+$2/($1+$2)*1150.)}')
...
pot_file = 'gap_files/pdauh.gap'

n_species = 3
species = Pd Au H
masses = 114.82 196.96 1.008
e0 = -1.46623303 -0.17939474 -1.11172070
...
mpirun -np 8 turbogap md &> /dev/null
